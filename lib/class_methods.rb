...
module ActsAsSolr #:nodoc:
  ...
  module ClassMethods
    ...
    def multi_model_suffix(options)     
      models = "AND (#{solr_configuration[:type_field]}:" + "#{solr_configuration[:app]}:#{self.name}".gsub!(":","_")
      models << " OR " + options[:models].collect {|m| "#{solr_configuration[:type_field]}:" + "#{solr_configuration[:app]}:#{m.to_s}".gsub!(":","_")}.join(" OR ") if options[:models].is_a?(Array)
      models << ")"
    end
